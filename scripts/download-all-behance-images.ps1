...
foreach ($g in $galleries) {
    ...
    try {
        ...
        $all = [regex]::Matches($res.Content, 'https://mir-s3-cdn-cf\.behance\.net/project_modules/(?:1400|1400_opt_1|fs|max_1200|disp)/[^\s"''\\]+?\.(?:jpg|png|jpeg|webp)')
        ...
        foreach ($u in ($urls | Select-Object -First 8)) {
            ...
            $downloaded += "/projects/$filename"
            $idx++
        }
        $imageManifest[$g.id] = $downloaded
    } catch {
        Write-Host "Error for $($g.id): $_"
    }
}

$json = $imageManifest | ConvertTo-Json -Depth 4
Set-Content -Path 'scripts/manifest.json' -Value $json
Write-Host "`nSaved all gallery images and manifest!"
